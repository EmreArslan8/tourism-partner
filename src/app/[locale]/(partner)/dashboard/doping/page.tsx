    ...
    <>
      ...
      {!biz ? (
        ...
        <div className="grid gap-4">
          {/* Mevcut durum */}
          <PartnerPanelCard bodyClassName="p-5" className={dopingActive || premium ? "border-line bg-cream/45" : ""}>
            <div className="flex items-center gap-2.5">
              <Rocket size={18} className={dopingActive || premium ? "text-sapphire" : "text-muted"} aria-hidden />
              <h2 className="text-[15px] font-medium text-ink">{t("dopingCurrentStatus")}</h2>
            </div>
            {premium ? (
              <p className="mt-3 inline-flex items-center gap-2 text-[14px] font-medium text-sapphire"><Crown size={16} aria-hidden /> {t("dopingPremiumActive")}</p>
            ) : dopingActive ? (
              <p className="mt-3 inline-flex items-center gap-2 text-[14px] font-medium text-sapphire">
          ...
          <div className="grid grid-cols-2 gap-4 max-[640px]:grid-cols-1">
            <PartnerPanelCard bodyClassName="p-4">
              <h3 className="inline-flex items-center gap-2 text-[14px] font-medium text-ink"><Crown size={15} className="text-sapphire" aria-hidden /> {t("dopingPremiumTitle")}</h3>
            ...
            <PartnerPanelCard bodyClassName="p-4">
              <h3 className="inline-flex items-center gap-2 text-[14px] font-medium text-ink"><Rocket size={15} className="text-sapphire" aria-hidden /> {t("dopingWelcomeTitle")}</h3>
          ...
          <div className="rounded-[10px] border border-dashed border-line bg-cream/45 p-4 text-[13px] text-muted">
            {t.rich("dopingFooter", {
              support: (chunks) => <Link href="/dashboard/support" className="font-medium text-sapphire hover:underline">{chunks}</Link>,
            })}
          </div>
        </div>
      )}
