...
const ApprovalBoard = ({ businesses, locale }: { businesses: AdminBusiness[]; locale: string }) => {
  ...
  const renderList = (key: TabKey) => {
    ...
    if (list.length === 0) {
      return (
        <p className="rounded-xl border border-dashed border-line bg-paper px-6 py-12 text-center text-[14px] text-muted/60">
...
const Card = ({ b, locale, lang }: { b: AdminBusiness; locale: string; lang: "tr" | "en" }) => {
  ...
  return (
    <div className="flex flex-col gap-5 rounded-xl border border-line bg-paper p-5 shadow-card lg:flex-row">
      {/* Kimlik + iletişim + detaylar */}
      <div className="flex min-w-0 flex-1 gap-4">
        <div className="grid h-12 w-12 shrink-0 place-items-center rounded-lg bg-cream text-brand">
        ...
        <div className="min-w-0">
          ...
          <div className="mt-2 space-y-1 text-[13px] text-muted">
            ...
            {detailRows.map((d) => (
              <p key={d.label}><span className="font-semibold text-muted">{d.label}:</span> {d.value}</p>
            ))}
          </div>
        </div>
      </div>
      ...
      {/* Evraklar */}
      <div className="lg:w-[280px] lg:shrink-0 lg:border-l lg:border-line lg:pl-5">
        <p className="mb-2 text-[11px] font-bold uppercase tracking-[.06em] text-muted/60">Yüklenen Evraklar</p>
        {docRows.length === 0 ? (
          <p className="text-[13px] text-muted/60">Tanımlı evrak yok.</p>
        ) : (
          <div className="space-y-2">
            {docRows.map((d, i) =>
              d.doc ? (
                <div key={i} className="flex items-center justify-between gap-2 rounded-lg border border-line px-3 py-2 text-[13px] text-ink">
                  <span className="flex min-w-0 items-center gap-2">
                    <Svg size={16}><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-6-6Zm0 0v6h6" /></Svg>
                    <span className="truncate">{d.label}</span>
                  </span>
                  {d.doc.url && (
                    <a href={d.doc.url} target="_blank" rel="noreferrer" className="text-muted hover:text-brand" aria-label="Görüntüle">
      ...
      <div className="flex shrink-0 flex-col gap-2 lg:w-[140px]">
        <form action={updateBusinessStatus}>
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-1.5 rounded-lg bg-emerald-600 px-4 py-2.5 text-[13px] font-semibold text-paper transition-colors hover:bg-emerald-700 disabled:cursor-not-allowed disabled:bg-emerald-300"
          >
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent title={`Reddet — ${name}`} description="Dilersen gerekçe ekle; işletmeye iletilmek üzere kaydedilir.">
        <form action={updateBusinessStatus} onSubmit={() => setOpen(false)} className="mt-4 grid gap-3">
          ...
          <textarea name="reason" rows={3} maxLength={500} placeholder="Reddetme gerekçesi (opsiyonel)" className="w-full rounded-lg border border-line bg-paper px-3 py-2 text-[14px] text-ink outline-none focus:border-red-400" />
          <div className="flex justify-end gap-2">
            ...
            <button type="submit" className="rounded-lg bg-red-600 px-4 py-2 text-[13px] font-semibold text-paper hover:bg-red-700">Reddet</button>
          </div>
        </form>
      </DialogContent>
    </Dialog>
