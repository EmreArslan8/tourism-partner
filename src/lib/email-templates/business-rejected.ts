...
type BusinessRejectedEmailArgs = {
  businessName: string;
  reason: string | null;
  dashboardUrl: string;
  logoUrl: string;
...
export function businessRejectedEmail({
  ...
  dashboardUrl,
  logoUrl,
}: BusinessRejectedEmailArgs) {
  const safeBusinessName = escapeHtml(businessName);
  const safeDashboardUrl = escapeHtml(dashboardUrl);
  const safeLogoUrl = escapeHtml(logoUrl);
  const trimmedReason = reason?.trim() || null;
  const subject = `${businessName} başvurusu hakkında`;

  const reasonBlock = trimmedReason
    ? `<tr>
                    <td style="padding:24px 38px 0;">
                      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="border-radius:12px;background:#fdf3f3;">
                        <tr>
                          <td style="padding:18px 20px;">
                            <div style="color:#9f1239;font-size:12px;font-weight:900;letter-spacing:0.7px;">RED GEREKÇESİ</div>
                            <div style="margin-top:7px;color:#334155;font-size:14px;line-height:23px;">${escapeHtml(trimmedReason)}</div>
                          </td>
                        </tr>
                      </table>
                    </td>
...
<html lang="tr">
  ...
  <body style="margin:0;padding:0;background:#f2f5fa;color:#0b102f;font-family:Arial,'Helvetica Neue',sans-serif;-webkit-font-smoothing:antialiased;">
                ...
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">
                  ...
                  <tr>
                    <td style="padding:30px 38px 0;">
                      <p style="margin:0;color:#334155;font-size:15px;line-height:25px;">Bu karar kesin değil. Panelinizden eksikleri tamamlayıp kaydettiğinizde başvurunuz otomatik olarak yeniden incelemeye alınır.</p>
                    </td>
                  </tr>${reasonBlock}
                  <tr>
                    <td style="padding:30px 38px 36px;">
                      ...
                      <p style="margin:18px 0 0;color:#7b8498;font-size:12px;line-height:19px;">Bu e-posta, Tourism Partner başvuru incelemesi sonrası otomatik olarak gönderildi.</p>
                    </td>
                  </tr>
                </table>
              </td>
    ...
    text: `Başvurunuz şu an onaylanmadı

${businessName} profili incelendi ve bu haliyle yayına alınamadı.
${trimmedReason ? `\nRed gerekçesi: ${trimmedReason}\n` : ""}
Bu karar kesin değil. Panelinizden eksikleri tamamlayıp kaydettiğinizde başvurunuz otomatik olarak yeniden incelemeye alınır.

Profili düzenle: ${dashboardUrl}
