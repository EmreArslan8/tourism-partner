...
const styles = {
  section:
    "relative flex h-full min-h-0 w-full flex-col justify-center gap-8 py-0 max-[900px]:h-auto max-[900px]:justify-start max-[640px]:!h-full max-[640px]:!justify-center max-[640px]:gap-0 max-[640px]:py-0",
  topGroup: "flex min-h-0 flex-col gap-5 max-[640px]:h-full max-[640px]:gap-4",
  head: "flex items-end justify-between gap-5 max-[640px]:shrink-0",
  headCopy: "section-copy max-w-[680px]",
  eyebrow: "eyebrow mb-1.5 text-brand/65",
  headTitle: "heading-section text-ink",
  lead: "section-desc max-w-[560px] font-medium",
  grid:
    "grid min-h-0 grid-cols-12 grid-rows-2 gap-5 max-[1180px]:grid-cols-6 max-[1180px]:grid-rows-none " +
    "max-[640px]:flex-1 max-[640px]:grid-cols-2 max-[640px]:grid-rows-[1.1fr_1fr_1fr] max-[640px]:gap-3",
  card:
    ...
    "max-[640px]:col-span-1 max-[640px]:min-h-0 max-[640px]:!aspect-auto max-[640px]:rounded-card " +
    "max-[640px]:first:col-span-2 max-[640px]:first:row-span-1 max-[640px]:first:min-h-0",
  ...
  arrow:
    ...
    "text-white opacity-0 backdrop-blur-md transition-all duration-300 group-hover:opacity-100 group-hover:translate-x-0 max-[640px]:hidden",
  body: "absolute inset-x-0 bottom-0 p-4 max-[640px]:p-4",
  name:
    "title-on-image mt-2 max-w-[11ch]",
  sidePanel:
    "relative flex shrink-0 flex-col gap-4 overflow-visible bg-transparent px-0 py-0 " +
    "max-[640px]:hidden",
  panelTitle: "heading-section text-ink",
  panelIntentList:
  ...
  panelIntentItem:
    ...
    "meta-text font-bold text-ink shadow-[inset_0_1px_0_rgba(255,255,255,.9),0_18px_38px_-30px_rgba(7,9,42,.75)] backdrop-blur-xl transition-all duration-200 " +
    "max-[640px]:min-h-[52px] max-[640px]:px-3.5 " +
