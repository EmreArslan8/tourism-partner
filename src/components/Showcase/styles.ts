...
const styles = {
  ...
  arrow:
    "grid h-11 w-11 place-items-center rounded-full border border-line bg-paper text-ink " +
  ...
  slide: "min-w-full px-0.5 py-2",
  panel:
    "grid grid-cols-[1.1fr_1fr] gap-5 " +
    "max-[860px]:grid-cols-1 max-[860px]:gap-0 max-[860px]:overflow-hidden max-[860px]:rounded-[18px] " +
    "max-[860px]:border max-[860px]:border-line max-[860px]:shadow-card",

  /* SOL — galeri (mobilde üst kısım, birleşik) */
  gallery:
    "relative min-h-[440px] overflow-hidden rounded-[20px] shadow-card " +
  ...
  info:
    "flex flex-col gap-4 rounded-[20px] border border-line bg-paper p-9 shadow-card " +
  ...
  name: "text-[clamp(22px,2.6vw,32px)] font-semibold leading-tight tracking-[-0.01em] text-ink max-[560px]:text-[19px]",
  ...
  desc: "line-clamp-3 text-[15px] leading-relaxed text-ink/70 max-[560px]:line-clamp-2 max-[560px]:text-[13.5px]",
  ...
  foot: "mt-auto flex items-center gap-4 border-t border-line pt-5 max-[560px]:gap-3 max-[560px]:pt-3.5",
  quote: "text-[14px] font-semibold text-terra hover:text-terra-deep",

  dots: "mt-5 flex items-center justify-center gap-2 max-[640px]:hidden",
