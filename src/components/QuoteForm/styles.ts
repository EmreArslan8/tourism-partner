/* QuoteForm — Tailwind sınıf token'ları */
 const styles= {
  ...
  supplierImage: "relative h-12 w-12 shrink-0 overflow-hidden rounded-[10px] border border-line",
  supplierImageInner: "object-cover",
  ...
  phoneField: "flex w-full min-w-0",
  phoneHeight: "h-[46px] min-[1440px]:h-[52px] min-[1800px]:h-[56px]",
  phoneInput:
    "field h-[46px] min-w-0 flex-1 rounded-s-none font-normal min-[1440px]:h-[52px] min-[1800px]:h-[56px]",
  ...
  dateLegend: "mb-1.5 block whitespace-nowrap text-[13px] font-semibold text-ink min-[1440px]:text-[14px] min-[1800px]:text-[14.5px]",
  /* Diğer alanlarla aynı kenarlık/yükseklik için `.field` token'ı — ayrı bir
     border/radius seti kullanınca koyu temada mor kenarlık farkı çıkıyordu. */
  dateTrigger:
    "field flex h-[46px] w-full items-center gap-3 text-start min-[1440px]:h-[52px] min-[1800px]:h-[56px]",
  dateTriggerText: "flex min-w-0 flex-1 items-center gap-2 font-normal",
  dateValue: "truncate font-normal text-ink [font-variant-numeric:tabular-nums]",
  datePlaceholder: "truncate text-muted",
  dateDash: "text-line",
  dateIcon:
    "grid h-4 w-4 shrink-0 place-items-center text-primary/70 [&_svg]:h-4 [&_svg]:w-4",
  datePopover:
    "absolute start-0 top-[calc(100%+8px)] z-30 w-[304px] rounded-[16px] border-[1.5px] border-ink/25 bg-paper p-3.5 shadow-[0_18px_46px_rgba(15,23,42,0.16)] max-[520px]:w-full",
